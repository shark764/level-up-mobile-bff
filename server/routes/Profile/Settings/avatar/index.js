...
const {
  ...
  error,
  handleImageResponse,
} = require('../../../../utils/helpers/response');
  ...
  (req, res) => {
    try {
      upload(req, res, async (err) => {
          ...
          return res
            ...
            .json(
              error({
                requestId: req.id,
                code: 400,
                message: err && HANDLE_BAD_REQUEST(err.message),
              })
            );
        ...
        console.log(req.user_id, req.file)
        ...
        console.log({ requestId: req.id, data: { user } })
      ...
      res
        ...
        .json(
          error({
            ...
            message: e.statusCode !== 404 && e.message,
          })
        );
    }
  }
